...
int main()
{
    ...
    {
        ListNode *list = singly_linked_list({3, 2, 0, -4});
        tail(list)->next = index(list, 1);

        const bool expected_output = true;
        const bool output = solution.hasCycle(list);

        Assert::equal(output, expected_output);
    }

    {
        ListNode *list = singly_linked_list({1, 2});
        tail(list)->next = index(list, 0);
    ...
    {
        ListNode *list = singly_linked_list({1});
        tail(list)->next = index(list, -1);
